...
import { 
  MOCK_AGENDAMENTOS, MOCK_PROFISSIONAIS, MOCK_BLOQUEIOS, 
  STATUS_LABELS, STATUS_COLORS, getClienteNome, getServicoNome, getServicoDuracao, getServicoPreco 
} from '@/lib/mock-data';
import type { Agendamento, StatusAgendamento } from '@/lib/gestao-types';
...
export default function AgendaPage() {
  ...
  const profissionais = MOCK_PROFISSIONAIS.filter(p => p.ativo);

  // Agendamentos e bloqueios do dia
  const doDia = agendamentos.filter(a => a.data === dataSelecionada);
  const bloqueiosDia = MOCK_BLOQUEIOS.filter(b => b.data_inicio.startsWith(dataSelecionada));
    ...
    <div className="py-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3 flex flex-col gap-3">
          {agendamentosFiltrados.length === 0 && bloqueiosDia.length === 0 ? (
            ...
            <>
              {/* Render Bloqueios (se houver, de forma simplificada por enquanto) */}
              {bloqueiosDia.map(b => (
                (profFiltro === 'todos' || profFiltro === b.profissional_id) && (
                  <div key={b.id} className="flex flex-col sm:flex-row gap-4 p-4 rounded-lg bg-foreground/[0.02] border border-[var(--border-subtle)] border-l-4 border-l-amber-500/50">
                    ...
                    <div className="flex-1 flex items-center">
                      <div>
                        ...
                        <div className="text-sm text-foreground/60">{MOCK_PROFISSIONAIS.find(p=>p.id===b.profissional_id)?.nome} · Motivo: {b.motivo}</div>
                      </div>
                    </div>
                  </div>
                )
              ...
              {agendamentosFiltrados.map(a => {
                ...
                const prof = MOCK_PROFISSIONAIS.find(p=>p.id===a.profissional_id)?.nome;
