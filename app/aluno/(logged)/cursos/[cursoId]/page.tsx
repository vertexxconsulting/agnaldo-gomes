...
export default function CursoDetalhesPage({ params }: { params: Promise<{ cursoId: string }> }) {
  ...
  // Obter módulos e aulas deste curso
  const modulos = MOCK_MODULOS.filter(m => m.curso_id === curso.id).sort((a,b) => a.ordem - b.ordem);
  const aulas = MOCK_AULAS.filter(a => modulos.some(m => m.id === a.modulo_id));
  
  // Calcular progresso
  const aulasConcluidas = MOCK_PROGRESSO_ALUNO.filter(p => p.concluida && aulas.some(a => a.id === p.aula_id)).length;
  const totalAulas = aulas.length;
  const progressPercent = totalAulas > 0 ? Math.round((aulasConcluidas / totalAulas) * 100) : 0;

  // Encontrar próxima aula para continuar
  const ultimaAulaAssistida = MOCK_PROGRESSO_ALUNO.slice().reverse().find(p => aulas.some(a => a.id === p.aula_id));
  let urlContinuar = `/aluno/cursos/${cursoId}/aulas/${aulas[0]?.id}`; // Default para primeira aula
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#141414] pb-20">
      ...
      {/* Lista de Módulos (Acordeão visualmente aberto por padrão) */}
              ...
              <div key={modulo.id} className="bg-white/5 border border-white/10 rounded-lg overflow-hidden">
                ...
                <div className="flex flex-col">
                  {aulasModulo.map((aula, idx) => {
                    const progress = MOCK_PROGRESSO_ALUNO.find(p => p.aula_id === aula.id);
